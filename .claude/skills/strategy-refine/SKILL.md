...
If `GH_REFINEMENT_REPO` is set, check whether any refinement PRs have been moved from Draft to Ready for Review:

```bash
python3 scripts/check_refinement_prs.py --artifacts-dir artifacts
```

Capture the JSON output. This script scans `artifacts/strat-tasks/` for strategies with `refinement_pr_url` set and `needs_attention=true`, checks each PR's status via GitHub API, and:
- **Draft PRs**: Logs `[WAITING]`, removes any stale refinement doc, and adds the strategy to the `waiting` list
- **Ready/merged PRs**: Fetches the refinement document to `artifacts/strat-refinements/{strat_id}-refinement.md` and adds to the `ready` list
- **Closed PRs**: Clears `refinement_pr_url` from frontmatter and removes any stale refinement doc

**Skip waiting strategies.** Any strategy ID in the `waiting` list from the JSON output MUST be skipped entirely during refinement — do not read it, do not revise it, do not process it. These strategies are parked until the staff engineer marks the PR Ready for Review.

For each strategy in the `ready` list, remove the `strat-creator-needs-attention` label from the Jira issue (if `jira_key` is set and not in dry-run mode):
...
- **No jira_key**: Skip label removal.
- **Jira credentials unavailable**: Skip label removal and notify the user.

If `GH_REFINEMENT_REPO` is not set, skip this step silently.
...
Each file also contains the business need from the source RFE. This business need is **fixed input — VERBATIM, character-for-character from the original RFE**. Do NOT paraphrase, rewrite, clean up grammar, or reinterpret the business need text. Your job is to add or revise the HOW (the `## Strategy` section), not change the WHAT.
...
Before generating the strategy, gather high-priority inputs. You can use the deterministic script:

```bash
python3 scripts/gather_refine_inputs.py {strat_id} \
    --source-rfe {source_rfe} \
    --artifacts-dir artifacts
```

This script checks for input sources and outputs JSON indicating what's available.

**Priority chain** (highest to lowest):
1. PR Refinement Document (per-strategy, staff engineer edited)
2. Staff Engineer / SME Input (per-strategy, human-authored)
3. Architecture Context Overlays (cross-strategy, human-authored corrections)
4. Removed RFE Context (per-RFE implementation details)
5. Architecture Context (generated platform docs)

### Source 1 (Highest Priority): PR Refinement Document

If `pr_refinement.available` is true, read the file at the reported path. This was fetched during the PR feedback check from a GitHub PR where a staff engineer edited the strategy directly.

When a PR refinement document exists:
- Treat it as the **highest-priority input** — it overrides removed RFE context (Source 2) and architecture context when they conflict.
- The document is the staff engineer's edited version of the strategy. Use it in its entirety as the authoritative strategy content — do not diff against the original.
- If the document contradicts your analysis, follow the staff engineer's direction and note the reasoning.

### Source 2: Removed Implementation Context from RFE

Read the strategy's frontmatter to get the `source_rfe` key (e.g., RHAIRFE-710). If `gather_refine_inputs.py` reported `removed_context.available` as true, read the file at the reported path. Otherwise, fetch fresh comments from Jira:

```bash
python3 ${CLAUDE_SKILL_DIR}/scripts/fetch_issue.py RHAIRFE-NNNN --fields comment --markdown
```

Parse the JSON output and write the comments to `artifacts/strat-originals/{source_rfe}-comments.md` (overwriting any existing file). Scan the comments for one containing the marker `[RFE Creator]` followed by "The following technical implementation details were removed from the RFE description during review." This comment contains implementation details that rfe-creator deliberately stripped from the RFE to keep it "open to how" — these details are explicitly described as "better suited for a RHAISTRAT."
...
The script also writes the raw RFE comments to `artifacts/strat-originals/{source_rfe}-comments.md` for reference.
...
Each strategy file has two top-level sections with strict ownership rules:
...
On initial refinement, generate the `## Strategy` section content. On revision runs, regenerate the `## Strategy` section content informed by review feedback, PR refinement documents, Staff Engineer / SME Input, and removed RFE context. Never touch the other two sections.
